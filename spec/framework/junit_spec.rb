...
describe TestOutputParser::Framework::JUnit do
  ...
  it 'should handle gradle output' do
    TestOutputParser::Framework::JUnit.count("47 tests completed, 1 failed").to_hash.should == { :total => 47, :failed => 1 }
  end

  it 'should ignore lines not related to JUnit test summary' do
    TestOutputParser::Framework::JUnit.count(File.read("spec/fixtures/sample-junit-output.txt")).to_hash.should == { :total => 13 }
  end

  it 'should include the failures in the test summary for JUnit output' do
    TestOutputParser::Framework::JUnit.count(File.read("spec/fixtures/sample-failed-junit-output.txt")).to_hash.should ==
      {
        :total => 13,
        :failed => 2,
        :failures => '' +
  ...
  it 'should include the failures in the test summary for gradle output' do
    TestOutputParser::Framework::JUnit.count(File.read("spec/fixtures/sample-gradle-output.txt")).to_hash.should ==
      {
        :total => 47,
        :failed => 1,
        :failures => ''+
